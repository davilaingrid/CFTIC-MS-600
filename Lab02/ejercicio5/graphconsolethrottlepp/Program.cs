...
namespace graphconsolethrottlepp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Usado en la tarea 1 al 7
            //var client = GetAuthenticatedHTTPClient(config, userName, userPassword);

            var client = GetAuthenticatedGraphClient(config, userName, userPassword);

            //Código de la segunda parte del ejercicio
             //retry strategy
            var stopwatch = new System.Diagnostics.Stopwatch();
            stopwatch.Start();

            //Código usado en la tarea 8
            ...
            //Código de la primera parte del ejercicio: Tarea 2 al 6
        ...
        private static GraphServiceClient GetAuthenticatedGraphClient(IConfigurationRoot config, string userName, SecureString userPassword)
       {
        ...
        private static Message GetMessageDetail(HttpClient client, string messageId, int defaultDelay = 2)
        {
            ...
            // add code hereConsole.WriteLine("...Response status code: {0}  ", clientResponse.StatusCode);
